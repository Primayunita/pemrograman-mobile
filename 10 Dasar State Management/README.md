...
[LINK REPOSITORY KODE PRAKTIKUM](https://github.com/Primayunita/src-week-10.git)
...
class _PlanScreenState extends State<PlanScreen> {
  ...
  Widget build(BuildContext context) {
   return Scaffold(
    ...
    appBar: AppBar(title: const Text('Master Plan Nita')),
    body: _buildList(),
...
![Gambar 1](img/praktikum1_langkah14.jpeg)
...
Langkah 4 dilakukan untuk membuat satu file (data_layer.dart) yang menggabungkan semua model agar import jadi lebih mudah dan tidak berulang-ulang. Dengan cara ini, kode juga jadi lebih rapi dan mudah dikembangkan kalau nanti ada tambahan model baru.
...
Variabel plan digunakan untuk menyimpan dan mengatur data task di aplikasi.
Sedangkan const Plan() digunakan sebagai nilai awal yang sederhana dan efisien saat aplikasi pertama kali dijalankan.
...
- initState() (Langkah 11) digunakan untuk menyiapkan controller saat widget mulai digunakan
- dispose() (Langkah 13) digunakan untuk membersihkan controller saat widget sudah tidak dipakai, agar aplikasi tetap aman dan tidak boros memori.
...
class _PlanScreenState extends State<PlanScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: _buildAddTaskButton(planNotifier, plan),
    );
  }

  Widget _buildAddTaskButton(ValueNotifier<Plan> notifier, Plan plan) {
    return FloatingActionButton(
      child: const Icon(Icons.add),
      onPressed: () {
        notifier.value = Plan(
...
PlanProvider adalah bentuk dari InheritedWidget, dan digunakan InheritedNotifier agar perubahan data bisa langsung memperbarui tampilan secara otomatis tanpa perlu setState().
...
Method tersebut digunakan untuk menghitung dan menampilkan progress task secara otomatis. Hal ini dilakukan agar logika perhitungan disimpan di model (Plan) sehingga kode menjadi lebih rapi, mudah dipakai ulang, dan tidak bercampur dengan tampilan (UI).
...
@override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Master Plans Nita')),
      body: Column(
        children: [
          _buildListCreator(),
...
![Gambar 1](img/praktikum3_langkah14.jpeg)
...
![Gambar 1](img/tugasprak3_no2.png)

*Jawab:*

Diagram tersebut menjelaskan alur aplikasi Flutter yang memiliki dua halaman. Halaman pertama (PlanCreatorScreen) digunakan untuk menambah dan melihat daftar plan. Setelah pengguna memilih salah satu plan, aplikasi berpindah ke halaman kedua (PlanScreen) menggunakan Navigator.push.

Di halaman kedua, pengguna bisa melihat dan mengatur task dari plan yang dipilih. Data plan disimpan menggunakan PlanProvider, sehingga tetap bisa digunakan saat berpindah halaman. Tampilan pada kedua halaman disusun menggunakan widget seperti Column, ListView, dan TextField agar lebih rapi dan mudah dipahami.
